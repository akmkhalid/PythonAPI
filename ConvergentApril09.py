...
class ConvergenceAnalyzer:
    ...
    def compute_ai_fingerprint(self, texts, is_baseline=False):
        """Compute comprehensive AI fingerprint with baseline filtering

        Args:
            texts: List of text strings to analyze
            is_baseline: If True, returns raw scores for baseline establishment.
                        If False, filters out common English words.
        """
        scores = []

        # Common English words that should NOT count as AI markers
        # These are normal human writing features that appear in 2003 texts
        common_english_words = {
            ...
            'give', 'day', 'most', 'us', 'very', 'here', 'would', 'should', 'could',
            'may', 'might', 'must', 'shall', 'will', 'has', 'have', 'had', 'been',
            'being', 'am', 'are', 'is', 'was', 'were', 'does', 'did', 'doing'
        }
        ...
        for text in texts:
            ...
            if is_baseline:
                # BASELINE MODE: Count ALL markers (for establishing baseline)
                ...
                # FILTERED MODE: Exclude common English words (human baseline)
                classic_count = sum(1 for marker in self.chatgpt_classic_markers
                                    if marker in text_lower and marker not in common_english_words)
                listicle_count = sum(1 for marker in self.listicle_markers
                                     if marker in text_lower and marker not in common_english_words)
                psychology_count = sum(1 for marker in self.psychology_terms
                                       if marker in text_lower and marker not in common_english_words)
            ...
            weighted_score = (
                    classic_count * 3.0 +  # Classic ChatGPT markers
                    listicle_count * 2.0 +  # Listicle structure
                    psychology_count * 3.0 +  # Psychology terms
                    contraction_count * 4.0 +  # Casual contractions
                    colloquial_count * 3.5 +  # Colloquial markers
                    business_count * 2.0 +  # Business jargon
                    empathetic_count * 3.0 +  # Empathetic language
                    exclamation_count * 3.0 +  # Exclamations
                    modifier_count * 1.5 +  # Overused modifiers
                    hallucination_count * 3.0 +  # Hallucination phrases
                    buzzword_count * 2.0 +  # Buzzwords
                    structure_count * 1.0 +  # Structure markers
                    genz_count * 4.0 +  # Gen-Z slang
                    fashion_count * 3.5 +  # Fashion markers
                    transitional_count * 3.0 +  # Transitional phrases
                    inspirational_count * 3.0 +  # Inspirational phrases
                    listicle_2026_count * 3.0  # 2026 listicle markers
            )
    ...
    def compute_all_metrics(self, texts, is_baseline=False, baseline_mean=None):
        """Compute all metrics with optional baseline normalization"""
        ...
        # AI fingerprint - with baseline filtering
        if baseline_mean is not None:
            # For non-baseline years: subtract baseline
            ai_scores_raw = self.compute_ai_fingerprint(cleaned_texts, is_baseline=False)
            ai_score = np.mean(ai_scores_raw) - baseline_mean
        else:
            # For baseline year: return raw (will be used to calculate baseline_mean)
            ai_scores_raw = self.compute_ai_fingerprint(cleaned_texts, is_baseline=True)
            ai_score = np.mean(ai_scores_raw)
        ...
        return {
            ...
            'ai_score': ai_score,
            'ai_score_raw': ai_scores_raw if baseline_mean is None else None,
...
def main():
    ...
    for genre in sorted(genres_all):
        ...
        # Store raw results for additional visualizations
        raw_results_by_year = {}
        ...
        for year in available_years:
            ...
            # For raw visualizations (store cleaned texts and results)
            cleaned_texts = [TextPreprocessor.clean_text(t) for t in texts]
            temp_analyzer = ConvergenceAnalyzer()
            raw_results_by_year[year] = temp_analyzer.analyze_year_group(texts, year)
        ...
        # ============================================================
        # TREND VISUALIZATION (4 subplots)
        # ============================================================
        fig, axes = plt.subplots(2, 2, figsize=(12, 10))
        ...
        plt.show()
        print(f"  ✓ Saved trend analysis plot")

        # ============================================================
        # ADDITIONAL VISUALIZATIONS
        # ============================================================
        additional_dir = f'{output_dir}/additional_plots'
        os.makedirs(additional_dir, exist_ok=True)

        visualizer = ConvergenceVisualizer()
        us_analyzer = US_Structural_Analyzer()

        # 1. Comparison bar charts (2003 vs 2025, 2003 vs 2026)
        if 2025 in raw_results_by_year:
            visualizer.plot_comparison(
                raw_results_by_year[2003], raw_results_by_year[2025],
                2003, 2025,
                output_dir=additional_dir
            )
            print(f"  ✓ Saved 2003 vs 2025 comparison plot")

        if 2026 in raw_results_by_year:
            visualizer.plot_comparison(
                raw_results_by_year[2003], raw_results_by_year[2026],
                2003, 2026,
                output_dir=additional_dir
            )
            print(f"  ✓ Saved 2003 vs 2026 comparison plot")
        ...
        if 2026 in raw_results_by_year:
            ...
            print(f"  ✓ Saved similarity distribution plots")

        # 3. US Structural Dominance plots
        texts_2003 = data_dict[2003][genre]
        us_results_2003 = us_analyzer.analyze_corpus(texts_2003)

        for year in [2025, 2026]:
            if year in available_years and year != 2003:
                ...
                if us_results_2003 and us_results_year:
                    us_analyzer.plot_us_dominance(
                        us_results_2003, us_results_year,
                        genre, additional_dir
                    )
                    print(f"  ✓ Saved US dominance plot for {year}")

        # 4. Fashion-specific pattern plot (only for Fashion genre)
        if genre == 'Fashion' and 2026 in raw_results_by_year:
            plot_fashion_convergence_pattern(raw_results_by_year[2003], raw_results_by_year[2026])
            print(f"  ✓ Saved fashion convergence pattern plot")
